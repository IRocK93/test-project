...
class _ModelDownloadScreenState extends ConsumerState<ModelDownloadScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(context.l10n.downloadAiModel), centerTitle: true),
      body: Center(
        child: Padding(
  ...
  Widget _buildNotStarted(BuildContext context, ModelDownloadNotifier notifier) {
    return Column(
      ...
      children: [
        ...
        Text(context.l10n.downloadAiModel, style: const TextStyle(fontSize: DesignTokens.fontXl2, fontWeight: FontWeight.w700)),
        const SizedBox(height: DesignTokens.spaceMd),
        Text(context.l10n.downloadAiModelDesc(_formatSize(widget.sizeBytes ?? 3427873600)), textAlign: TextAlign.center, style: TextStyle(fontSize: DesignTokens.fontMd2, color: context.textSecondary, height: 1.5)),
        const SizedBox(height: DesignTokens.spaceSm),
        Row(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(PhosphorIconsLight.shieldCheck, size: 16, color: context.colorScheme.primary), const SizedBox(width: 6), Text(context.l10n.runsOnDevice, style: TextStyle(fontSize: DesignTokens.fontSm2, color: context.colorScheme.primary, fontWeight: FontWeight.w600))]),
        const SizedBox(height: DesignTokens.space3xl),
        SizedBox(width: double.infinity, height: 52, child: ElevatedButton(onPressed: () { debugPrint('[DOWNLOAD] Start Download tapped: ${widget.url}'); notifier.startDownload(url: widget.url, destinationPath: widget.destinationPath, expectedSha256: widget.sha256, version: widget.version); }, style: ElevatedButton.styleFrom(backgroundColor: context.colorScheme.primary, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(DesignTokens.radiusMd))), child: Text(context.l10n.startDownload, style: const TextStyle(fontSize: DesignTokens.fontLg, fontWeight: FontWeight.w600, color: Colors.white)))),
        const SizedBox(height: DesignTokens.spaceMd),
        TextButton(onPressed: () { if (widget.onSkip != null) { widget.onSkip!(); } else { Navigator.pop(context); } }, child: Text(context.l10n.skipForNow, style: TextStyle(color: context.textCaption))),
      ],
    );
  }
  ...
  Widget _buildProgress(double progress, int downloaded, int total, ModelDownloadNotifier notifier) {
    return Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      const SizedBox(height: DesignTokens.space2xl), Text(context.l10n.downloadingAiModel, style: const TextStyle(fontSize: DesignTokens.fontLg, fontWeight: FontWeight.w600)),
      const SizedBox(height: DesignTokens.spaceSm), Text('${_formatSize(downloaded)} / ${_formatSize(total)}', style: TextStyle(fontSize: DesignTokens.fontMd, color: context.textSecondary)),
      const SizedBox(height: DesignTokens.space2xl),
      TextButton.icon(onPressed: () { notifier.cancelDownload(); Navigator.pop(context); }, icon: const Icon(PhosphorIconsLight.x, size: 18), label: Text(context.l10n.cancel)),
    ]);
  }
  ...
  Widget _buildVerifying() {
    return Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      Text(context.l10n.verifyingDownload, style: const TextStyle(fontSize: DesignTokens.fontLg, fontWeight: FontWeight.w600)),
      const SizedBox(height: DesignTokens.spaceSm), Text(context.l10n.checkingIntegrity, style: TextStyle(fontSize: DesignTokens.fontMd, color: context.textSecondary)),
    ]);
  }
  ...
  Widget _buildComplete() {
    return Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      const SizedBox(height: DesignTokens.space2xl), Text(context.l10n.downloadComplete, style: const TextStyle(fontSize: DesignTokens.fontXl2, fontWeight: FontWeight.w700)),
      const SizedBox(height: DesignTokens.spaceSm), Text(context.l10n.aiCompanionReady, style: TextStyle(fontSize: DesignTokens.fontMd2, color: context.textSecondary)),
    ]);
  }
  ...
  Widget _buildError(BuildContext context, String message, ModelDownloadNotifier notifier) {
    return Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      const SizedBox(height: DesignTokens.space2xl), Text(context.l10n.downloadFailed, style: const TextStyle(fontSize: DesignTokens.fontLg2, fontWeight: FontWeight.w700)),
      ...
      const SizedBox(height: DesignTokens.space2xl),
      ElevatedButton(onPressed: () => notifier.startDownload(url: widget.url, destinationPath: widget.destinationPath, expectedSha256: widget.sha256, version: widget.version), style: ElevatedButton.styleFrom(backgroundColor: context.colorScheme.primary, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(DesignTokens.radiusMd))), child: Text(context.l10n.tryAgain, style: const TextStyle(fontWeight: FontWeight.w600, color: Colors.white))),
      const SizedBox(height: DesignTokens.spaceSm),
      TextButton(onPressed: () { if (widget.onSkip != null) { widget.onSkip!(); } else { Navigator.pop(context); } }, child: Text(context.l10n.useContentCards, style: TextStyle(color: context.textCaption))),
    ]);
  }
